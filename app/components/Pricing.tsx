...
const pricingPlans = [
  {
    ...
    price: '€350',
    priceDetail: 'pro Bereich',
    features: [
      'Beratung inklusive',
      'Hält 3-4 Monate',
      'Schnelle 15-minütige Behandlung',
      'Nachuntersuchung',
      'Verfügbar für Stirn, Krähenfüße und Zornesfalten'
    ],
    popular: false
  },
  {
  ...
  },
  {
    ...
    features: [
      ...
      'Empfohlene Serie von 3 Behandlungen',
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {pricingPlans.map((plan, index) => (
            <motion.div
              ...
              className={`price-card relative ${plan.popular ? 'ring-2 ring-[#D0D0D0]' : ''}`}
            >
              {plan.popular && (
                <div className="absolute -top-4 left-1/2 transform -translate-x-1/2 bg-[#C0A080] text-white px-4 py-1 rounded-full text-sm font-bold">
                  Beliebteste Wahl
                </div>
              )}
              ...
              <ul className="space-y-3 mb-8">
                {plan.features.map((feature, i) => (
                  <li key={i} className="flex items-start">
                    <svg className="h-6 w-6 text-[#C0A080] flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
